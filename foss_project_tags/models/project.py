    ...
    def _project_task_status(self):
        for val in self:
            ...
            if val.date_start and val.date:
                ...
                elif val.state == 'pending':
                    project_status = 'onhold'
                elif val.state == 'open' and today >= (date_end + relativedelta(days=-2)):
                    project_status = 'at_risk'
                elif not val.state == 'pending' and datetime.now().date() > date_end:
                    project_status = 'off_track'
                elif val.state == 'close':
                    project_status = 'done'
                else:
...
class Task(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get('project_id'):
            ...
            if vals.get('date_deadline') and vals.get('starting_dt'):   
                date_deadline = datetime.strptime(vals.get('date_deadline'), '%Y-%m-%d').date()
    ...
    state = fields.Selection([
        ...
        ('done', 'Completed'),
        ('cancelled', 'Cancelled'),
        ], string="Status", readonly=False, store=True,
    ...
    def onchange_check_date(self):
        if self.date_start and self.date_deadline:
            ...
            print("---------------------------->start",self.date_deadline)
